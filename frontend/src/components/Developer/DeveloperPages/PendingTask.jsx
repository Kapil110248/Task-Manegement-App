...
function PendingPage() {
  ...
  const pendingTasks = tasks.filter(
    (task) =>
      task.status === "Pending" &&
      String(task.developerId) === String(developerId)
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {pendingTasks.length === 0 ? (
        <p className="text-center text-gray-500">No tasks in pending</p>
      ) : (
          ...
          <table className="min-w-full bg-white text-sm rounded-xl">
            <thead className="bg-gray-100 text-gray-700">
              <tr>
                ...
                <th className="py-3 px-4 text-left">Pending Time</th>
              ...
              {pendingTasks.map((task, index) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <span className="bg-yellow-400 text-white px-3 py-1 text-xs rounded-full font-semibold">
                      {task.status}
                    </span>
                  </td>
                </tr>
